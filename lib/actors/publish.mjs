...
/**
 * 발행/미리보기 공용 — images 도메인 data(roles[]) 와 계승 도메인을 검증·수집한다(쓰기 없음).
 * @returns {{ ok:true, ch, lore, roles, totalScenes, totalRanges } | { ok:false, action, storyId, reason? }}
 */
export function buildImageDomainData(storyId) {
  ...
  return { ok: true, ch, lore: baseManifest?.domains?.lore ?? { source: 'legacy-live' }, roles, totalScenes, totalRanges };
}

/**
 * @returns {{ ok, action, storyId, releaseId?, roles?, scenes?, ranges?, reason? }}
 */
export function publishActorRelease(storyId) {
  const built = buildImageDomainData(storyId);
  if (!built.ok) return { ...built, storyId };
  const { ch, lore, roles, totalScenes, totalRanges } = built;
