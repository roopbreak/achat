...
import {
  ...
  getActor, getActorAssets, getActorInheritance,
  replaceResolvedScenes,
} from '../db.mjs';
import {
  flattenActorScenes, flattenActorRules, applyOverrides, resolveOutputRules, serializeResolvedRules,
  computeAssetLocator, computeInputFingerprint, computeActorAssetsFingerprint,
} from './flatten.mjs';
...
export function materializeStoryCharacter(storyCharacterId) {
  ...
  for (const b of bindings) {
    ...
    // input_fingerprint = 해소된 출력(scenes + role_dir + 규칙)의 해시.
    const fp = computeInputFingerprint(b.role_dir, ruleText, sceneRows);
    for (const r of sceneRows) {
      allRows.push({ ...r, resolved_rule_text: ruleText, input_fingerprint: fp });
    }
  }

  replaceResolvedScenes(storyCharacterId, allRows);
  return { storyCharacterId, bindings: bindings.length, scenes: allRows.length };
}
