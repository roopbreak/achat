...
export async function embed(text) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[embed] Voyage API ${res.status}: ${await res.text().catch(() => '')}`);
      return null;
    }
...
export async function searchMemory(query, candidates, topK = 8) {
  if (!candidates.length || !VOYAGE_API_KEY) return [];

  const queryVec = await embed(query);
  if (!queryVec) return [];

  return searchMemoryWithVec(queryVec, candidates, topK);
}

export function searchMemoryWithVec(queryVec, candidates, topK = 8) {
  ...
  const scored = candidates
    ...
    .filter(r => r && r.score >= MIN_SIMILARITY)
