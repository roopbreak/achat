 ...
 * floor 미달이어도 RATIO 이상이면 그대로 두고, RATIO 미만(정말 짧은 것)만 보강.
 */
...
export async function streamWithContinuation({ systemBlocks, messages, res, model, maxTokens }) {
  ...
  for (let attempt = 0; attempt <= MAX_CONTINUE; attempt++) {
    ...
    // 종료 판정 (하한·임계 모두 꼬리 제외 본문 기준 — 상태창 글자수로 부풀려지지 않게)
    if (isTerminal(result.finishReason)) break;        // 필터/오류 — 더 시도 무의미
    // 본문이 하한 충족 → 잘림이든 완결이든 종료(잘림 과잉 이어쓰기로 인한
    // 멀티턴 폭주 차단 — 2026-06-10).
    if (body.length >= floor) break;
    // 정상 종료(stop): 본문이 floor*RATIO 이상이면 짧아도 그대로 둔다.
    // 어중간한 완결에 이어쓰기를 걸면 주인공 턴 침범 폭주(3번안 핵심).
    if (!truncated && body.length >= floor * STOP_CONTINUE_RATIO) break;
