...
import { OUTPUT_FLOORS, DEFAULT_OUTPUT_BAND, resolveOutputBand } from '../prompt/builtins.mjs';

// 본문 글자수 하한(floor)은 OUTPUT_FLOORS(밴드별 — builtins.mjs)로 단일화.
// 목표 밴드 하한과 일치(three-part-separation §3-6b 보강 3 — 구 CONTINUE_FLOORS 는
// 목표 하한보다 낮아 발동해도 목표 미달인 채 멈추는 누수가 있었다).
// 판정은 상태창 제외 본문 길이 기준.
 ...
 * 0.5 → 0.7 상향(§3-6b — 실효 하한 누수의 직접 원인. 일괄 1.0 금지, 로그 관측 후 조정).
 */
const STOP_CONTINUE_RATIO = 0.7;
 ...
 */
export async function streamWithContinuation({ systemBlocks, messages, res, model, maxTokens, band, autoContinue = true, cacheTtl }) {
  const provider = getGenerationProvider(model);
  const resolvedBand = resolveOutputBand(band ?? maxTokens);
  const floor = OUTPUT_FLOORS[resolvedBand] ?? OUTPUT_FLOORS[DEFAULT_OUTPUT_BAND];
  // 사용자 토글(off) 시 이어쓰기 미발동 — 1회 생성으로 종료(짧으면 짧은 대로, ~~ 로 이어가기).
  const maxContinue = autoContinue === false ? 0 : MAX_CONTINUE;
  ...
  for (let attempt = 0; attempt <= maxContinue; attempt++) {
    if (attempt > 0) {
      continued = true;
    ...
    const result = await provider.stream({ systemBlocks, messages: workingMessages, res, model, maxTokens, segmentIndex: attempt, cacheTtl });
  ...
  // 분량 관측 로그(매 턴 — §3-6b): 미달이 stop 인지 length 인지 게이트 미발동인지
  // 데이터로 추적한다. 프롬프트 레버(R1~R4)·면제선 조정의 근거.
  console.log(`[output] model=${model} band=${resolvedBand} body_chars=${body.length} floor=${floor} finish=${lastFinish} segments=${segments.length} continued=${continued} status=${status ? 'yes' : 'none'} out_tokens=${totalOutput}`);
  ...
  return {
    ...
    segments,
    providerMeta: { continued, segmentCount: segments.length, floor, band: resolvedBand, bodyChars: body.length },
  };
}
