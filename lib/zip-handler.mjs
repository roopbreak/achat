...
export function importFromZip(storyName, zipPath) {
  ...
  for (const entry of imageEntries) {
    ...
    // 저장 파일명: sceneKey 기반으로 단순화 (타임스탬프 제거)
    const ext      = path.extname(filename).toLowerCase();
    const saveName = filename; // 원본 파일명 유지
    const dest     = path.join(destDir, saveName);

    zip.extractEntryTo(entry, destDir, false, true);
    insertStoryImage(storyName, charDir, sceneKey, saveName);
    saved++;
  }
