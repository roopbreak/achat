...
// ── Story system commands (`!`-시스템 명령어 — plan three-part-separation §3-2) ──
// 가이드 커맨드(commands — LLM 입력 안내)와 별개. 앱이 인터셉트하는 기능 트리거.
// 스토리 종속: 코드 내장 기본 세트 + 스토리별 추가/오버라이드(enabled:false=숨김).

const SYSCMD_MAX_ITEMS = 30;
const SYSCMD_KINDS = ['client_toggle', 'server_action', 'mode_toggle', 'prompt_command'];
const SYSCMD_ACTION_RE = /^[a-zA-Z][a-zA-Z0-9_]{0,39}$/;
// kind 별 action 화이트리스트 — 구현이 존재하는 액션만 등록 허용(Codex high 2:
// 저장은 통과하고 실행 시 400 나는 "서버가 모르는 action" 등록을 저장 단계에서 차단).
// mode_toggle 은 자유 키(스토리 고유 모드 — directive 주입). lib/commands/builtins.mjs 와 동기.
// prompt_command = 카드 정의 LLM 커맨드(클릭 시 trigger 텍스트를 그대로 전송 — 인터셉트 아님).
const SYSCMD_KIND_ACTIONS = {
  client_toggle: ['debugPanel'],
  server_action: ['summarize'],
  prompt_command: ['send'],
};

function isValidSystemCommand(c) {
  return c && typeof c === 'object'
    && typeof c.trigger === 'string' && /^![^\s!]\S{0,29}$/.test(c.trigger.trim())
    && typeof c.label === 'string' && c.label.trim() !== ''
    && SYSCMD_KINDS.includes(c.kind)
    && typeof c.action === 'string' && SYSCMD_ACTION_RE.test(c.action)
    && (c.kind === 'mode_toggle' || SYSCMD_KIND_ACTIONS[c.kind].includes(c.action));
}

function normalizeSystemCommand(c) {
  const out = {
    trigger: c.trigger.trim(),
    label: c.label.trim().slice(0, 30),
    kind: c.kind,
    action: c.action,
  };
  if (typeof c.desc === 'string' && c.desc.trim() !== '') out.desc = c.desc.trim().slice(0, 200);
  if (c.requiresArg === true) out.requiresArg = true;
  if (c.enabled === false) out.enabled = false;
  // directive(모드 on 시 프롬프트 주입 텍스트)는 커스텀 mode_toggle 전용
  if (c.kind === 'mode_toggle' && typeof c.directive === 'string' && c.directive.trim() !== '') {
    out.directive = c.directive.trim().slice(0, 2000);
  }
  return out;
}

function cleanSystemCommands(arr) {
  if (!Array.isArray(arr)) return [];
  const seen = new Set();
  return arr.filter(isValidSystemCommand).map(normalizeSystemCommand)
    .filter(c => !seen.has(c.trigger) && seen.add(c.trigger))
    .slice(0, SYSCMD_MAX_ITEMS);
}

export function parseSystemCommands(raw) {
  if (Array.isArray(raw)) return cleanSystemCommands(raw);
  if (typeof raw !== 'string' || raw.trim() === '') return [];
  try { return cleanSystemCommands(JSON.parse(raw)); } catch { return []; }
}

export function serializeSystemCommands(input) {
  let arr = input;
  if (typeof input === 'string') {
    try { arr = JSON.parse(input); } catch { return '[]'; }
  }
  return JSON.stringify(cleanSystemCommands(arr));
}

// ── Session mode flags (mode_toggle 상태 — chat_sessions.mode_flags JSON) ──

export function parseModeFlags(raw) {
  if (typeof raw !== 'string' || raw.trim() === '') return {};
  try {
...
export function setSessionModeFlag(sessionId, action, on) {
  const session = db.prepare('SELECT mode_flags FROM chat_sessions WHERE id = ?').get(sessionId);
  if (!session) return null;
  const flags = parseModeFlags(session.mode_flags);
  if (on) flags[action] = true;
  else delete flags[action];
  db.prepare('UPDATE chat_sessions SET mode_flags = ?, updated_at = unixepoch() WHERE id = ?')
...
export function updateStory(id, fields) {
  ...
  const allowed = [
    'title', 'char_name', 'description', 'personality', 'scenario', 'first_mes',
    'post_history_instructions', 'category', 'tags', 'narration_style', 'narration_style_source', 'commands',
    // 응답 구성(011 — three-part-separation): 검증은 아래에서 화이트리스트로
    'status_mode', 'choices_mode', 'output_target', 'system_commands',
  ];
  const sets = [];
  const vals = [];
  for (const key of allowed) {
    if (key in fields) {
      ...
      if (key === 'system_commands') val = serializeSystemCommands(fields[key] ?? []);
      // 응답 구성 enum 검증 — 불량 값은 해당 필드 무시(NOT NULL 컬럼 보호)
      if (key === 'status_mode' && !['off', 'bottom', 'top'].includes(val)) continue;
      if (key === 'choices_mode' && !['on', 'off'].includes(val)) continue;
      if (key === 'output_target' && val !== null && typeof val !== 'string') continue;
      sets.push(`${key} = ?`);
      vals.push(val);
    }
  }
...
/**
 * 스토리별 나이 오버라이드만 갱신(별도 함수 — persona_id/override 와 독립).
 * setStoryPersona 가 전체 덮어쓰기라, 나이를 거기 얹으면 채팅 페르소나 변경 시
 * 미전달로 소실된다(Codex critical). null = 오버라이드 해제.
 */
export function setStoryPersonaAge(storyId, age) {
  return db.prepare(
    'UPDATE stories SET persona_age_override=?, updated_at=unixepoch() WHERE id=?'
  ).run(age == null ? null : age, storyId);
}
