...
export function initDB(dbPath) {
  ...
  try { db.exec('ALTER TABLE stories ADD COLUMN commands TEXT'); } catch {}
  try { db.exec('ALTER TABLE lore_entries ADD COLUMN embedding TEXT'); } catch (e) {
...
export function upsertStory({ name, char_name, description, personality, scenario, first_mes, post_history_instructions, narration_style, narration_style_source, commands }) {
  const nfcName = name.normalize('NFC');
  // commands는 INSERT 시에만 설정 — 재임포트(ON CONFLICT) 시 수동 등록된 커맨드 보존
  return db.prepare(`
    INSERT INTO stories (name, char_name, description, personality, scenario, first_mes, post_history_instructions, narration_style, narration_style_source, commands, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
...
// ── Story commands (스토리 전용 !커맨드) ────────────────────
// commands 컬럼은 TEXT에 JSON 배열을 저장. 항목 shape: { cmd, desc, group? }

// 크기 상한 — 비정상적으로 큰 입력으로 저장·조회 비용이 폭증하는 것을 방지
...
/** commands raw 값 → 항상 배열. JSON 오류·non-array·shape 오염 시 [] */
...
/** commands 입력(배열 또는 JSON 문자열) → 저장용 JSON 문자열. 전체 실패 시 '[]' */
...
export function updateStory(name, fields) {
  const allowed = ['char_name', 'description', 'personality', 'scenario', 'first_mes', 'post_history_instructions', 'category', 'tags', 'narration_style', 'narration_style_source', 'commands'];
  const sets = [];
  const vals = [];
...
export function createStoryManual({ name, char_name, description, personality, scenario, first_mes, post_history_instructions, category, tags, narration_style, narration_style_source, commands }) {
  return db.prepare(`
    INSERT INTO stories (name, char_name, description, personality, scenario, first_mes, post_history_instructions, category, tags, narration_style, narration_style_source, commands, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
...
export function updateLoreEntry(id, fields) {
  ...
  if (!sets.length) return;
  // content 변경 시 기존 embedding 무효화 (재임베딩 전까지 stale 벡터 방지)
...
export function getUnembeddedLore(storyName) {
  return db.prepare(
    'SELECT * FROM lore_entries WHERE story_name = ? AND enabled = 1 AND embedding IS NULL ORDER BY insertion_order'
  ).all(storyName);
}
