...
export function upsertStory({ name, char_name, description, personality, scenario, first_mes, post_history_instructions }) {
  return db.prepare(`
    INSERT INTO stories (name, char_name, description, personality, scenario, first_mes, post_history_instructions, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, unixepoch())
  ...
  `).run(name, char_name, description, personality ?? null, scenario ?? null, first_mes ?? null, post_history_instructions ?? '');
}
...
export function updateStory(name, fields) {
  const allowed = ['char_name', 'description', 'personality', 'scenario', 'first_mes', 'post_history_instructions', 'category', 'tags'];
  const sets = [];
  const vals = [];
...
export function createStoryManual({ name, char_name, description, personality, scenario, first_mes, post_history_instructions, category, tags }) {
  return db.prepare(`
    INSERT INTO stories (name, char_name, description, personality, scenario, first_mes, post_history_instructions, category, tags, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
