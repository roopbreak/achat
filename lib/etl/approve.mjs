...
import { computeFingerprint, validatePayload } from './extract.mjs';
...
export function approveStory(storyId) {
  ...
  // F2: 플래그(irrecoverable/unresolved)를 비워도 payload 자체가 온전하지 않으면 승인 거부.
  // → 검토자가 교정 없이 안전장치만 해제하는 우회를 차단. 손상 frozen release 발행 방지.
  const payloadErrors = validatePayload(payload);
  if (payloadErrors.length) {
    return { ok: false, action: 'invalid-payload', storyId, reason: payloadErrors.join('; ') };
  }
