...
- \`~행동~+\` : \`~행동~\`과 동일하되(주인공 행동 명시), 그 행동에 대해 **NPC가 현재 단계 안에서 능동적으로 연쇄 반응**(혀·손길·의류 위 더듬기 등)하는 것을 허용. 단 **다음 단계 진입 금지**, 주인공의 추가 행동·대사 임의 생성도 여전히 금지. (\`~~\`는 주인공 행동이 없을 때, \`~행동~+\`는 주인공 행동이 명시됐고 NPC의 능동 반응을 원할 때.)
- 일반 텍스트 : 주인공의 대사.
...
- **한 턴 한 단계**: 단계 = ①키스 ②애무 ③전희 ④삽입 ⑤절정 중 **하나만**. 다음 단계로 넘어가지 않고 같은 단계 안에서 밀도를 높인다. **같은 단계 내부에서는 NPC의 신체 반응·자발적 손길·의류 위 더듬기 등을 다층으로 전개할 수 있다**(예: 키스 단계 → 혀·손길·옷 위 더듬기까지 허용, 단 애무·전희 단계로 진입 금지). 같은 단계 내부라도 시간 점프나 요약 서술로 감각 전개를 건너뛰지 않는다. 장면 끝은 열린 상태로 남긴다.
...
- **게이지 낮을수록 NPC 수동**: 게이지가 낮으면(대략 하위 1/3) NPC는 받기·반응 위주이며 스스로 옷을 벗지 않는다. 게이지가 오를수록 자발적 손길·의류 해제 등 능동성이 커진다. 단 게이지가 높아도 "한 턴 한 단계" 원칙은 유지 — 능동성은 **현재 단계 내 밀도**로 표현하고 단계 건너뛰기로 표현하지 않는다.
...
export async function buildContext(story, sessionId, userInput, maxTokens = 3072, options = {}) {
  ...
  // 자동 이미지 카탈로그 주입 — story_images에 등록된 이미지가 있으면 항상 주입
  // (description에 placeholder/외부URL이 있어도 자체 카탈로그가 있으면 AI가 둘 다 활용)
  // story_images가 비어있는 외부URL 전용 스토리는 buildImageSection이 빈 문자열 반환.
  const composition = loadComposition(story.slug);
  const imageSection = buildImageSection(story.slug, imageIndex, composition);
