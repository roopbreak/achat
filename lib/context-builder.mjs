 ...
 * @param {{ content: string }[]} recentMsgs - 최근 메시지 (역순 아님)
 * @param {string} userInput - 현재 유저 입력
 * @param {number} tokenBudget
 */
function matchLore(allLore, recentMsgs, userInput, tokenBudget = LORE_TOKEN_BUDGET) {
...
export async function buildContext(storyName, sessionId, userInput, maxTokens = 4096, options = {}) {
  ...
  // ── 동적 컨텍스트: 키워드 매칭 로어북 ────────────────────────
  const matchedLore = matchLore(allLore, activeMsgs, userInput);
