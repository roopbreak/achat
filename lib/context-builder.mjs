import {
  ...
  getStoryImageIndex, getStoryNote, getPersona,
} from './db.mjs';
...
export async function buildContext(storyName, sessionId, userInput) {
  ...
  const personaText = persona
    ? `## 플레이어 캐릭터\n${story.persona_override ?? persona.content}`
    : null;
  ...
  const block1Text = [charSection, personaText, NARRATION_RULES, imageSection, noteText].filter(Boolean).join('\n\n');
