...
/**
 * sceneKey 목록 → 카테고리별 이미지 섹션 텍스트 생성
 */
function buildImageSection(storyName, sceneKeys) {
  ...
  for (const key of sceneKeys) {
    if (key.startsWith('crop-')) {
      ...
      key.includes('beach') || key.includes('room') || key.includes('office') ||
      key.includes('cafe') || key.includes('school') || key.includes('outdoor')
    ) {
      groups['장소/상황'].push(key);
    } else if (
      key.includes('kiss') || key.includes('sex') || key.includes('hug') ||
      key.includes('fingering') || key.includes('breast') || key.includes('deepkiss')
    ) {
      groups['행위'].push(key);
    } else {
      groups['기타'].push(key);
  ...
  const lines = [
    ...
    `형식: ![](/images/${encodeURIComponent(storyName)}/SCENE_KEY)`,
    '',
    '[이미지 목록]',
  ];
 ...
 */
export async function buildContext(storyName, sessionId, userInput) {
  const story   = getStory(storyName);
  if (!story) throw new Error(`스토리 없음: ${storyName}`);

  const session    = getSession(sessionId);
  const sceneKeys  = getStorySceneKeys(storyName);
  ...
  const imageSection = buildImageSection(storyName, sceneKeys);

  const block1Text = [charSection, NARRATION_RULES, imageSection].filter(Boolean).join('\n\n');
