import {
  getEffectiveConstantLore, getEffectiveAllLore, getEffectiveEmbeddedLore,
...
import { resolveRelease } from './story-resolver.mjs';
import { buildActorCatalogText } from './actors/catalog.mjs';
...
// ── 정규식 키 (WS-F P3c) ─────────────────────────────────
// 키가 `/pattern/` 또는 `/pattern/flags`(i·u 만 유효, g 는 strip — test() lastIndex 상태 오염 방지)
// 형식이면 정규식으로 매칭한다. 컴파일 실패·과長 패턴은 무시(평문 fallback 아님 — 키 자체 무효).
// ⚠️ flags 는 [giu] 만 인정(Codex b3k9o6ui3) — `[a-z]*` 로 받으면 평문 키 '/foo/bar' 가
//    pattern=foo, flags=bar 로 오인돼 정규식으로 둔갑한다. 그 외 문자는 형식 불일치 → 평문 처리.
const REGEX_KEY_RE = /^\/(.+)\/([giu]*)$/s;
const MAX_REGEX_PATTERN_LEN = 200;
const regexKeyCache = new Map(); // 'pattern flags' → RegExp | null(컴파일 실패)

function parseRegexKey(trimmed) {
  const m = REGEX_KEY_RE.exec(trimmed);
  if (!m) return null;                       // 정규식 키 형식 아님
  const [, pattern, rawFlags] = m;
  if (pattern.length > MAX_REGEX_PATTERN_LEN) return { invalid: true };
  const flags = [...new Set(rawFlags)].filter(f => f === 'i' || f === 'u').join('');
  const cacheKey = `${pattern} ${flags}`;
  if (regexKeyCache.has(cacheKey)) {
    const cached = regexKeyCache.get(cacheKey);
    return cached ? { re: cached } : { invalid: true };
  }
  try {
    const re = new RegExp(pattern, flags);
    regexKeyCache.set(cacheKey, re);
    return { re };
  } catch {
    regexKeyCache.set(cacheKey, null);
    return { invalid: true };
  }
}

/**
 * 키워드 매칭: AND(+) / NOT(-) / ANY(기본) + 정규식 키(`/pattern/flags`) 지원
 * NOT prefix는 `-`를 사용 (기존 `!command` 트리거와 충돌 방지). `-/pattern/` = 정규식 NOT.
 * 정규식 키는 `+` AND 분리 비적용(패턴 내 `+` 충돌 — lookahead 로 표현).
 */
function keywordMatch(text, keysStr) {
  ...
  // NOT 조건 먼저 (-prefix): 하나라도 매칭되면 불매칭
  for (const k of keys) {
    const trimmed = k.trim();
    if (!trimmed.startsWith('-') || trimmed.length <= 1) continue;
    const body = trimmed.slice(1);
    const rx = parseRegexKey(body);
    if (rx) {
      if (rx.re && rx.re.test(text)) return false;
    } else if (lower.includes(body.toLowerCase())) {
      return false;
    }
  }
  ...
  for (const k of keys) {
    const trimmed = k.trim();
    if (trimmed.startsWith('-') && trimmed.length > 1) continue;
    const rx = parseRegexKey(trimmed);
    if (rx) {
      if (rx.re && rx.re.test(text)) return true;
...
export async function buildContext(story, sessionId, userInput, maxTokens = 3072, options = {}) {
  ...
  // WS-L/P3b-2: 세션이 핀한 release manifest 로 도메인별 source 해석(release_id NULL=legacy 무변경).
  // characters v2-frozen → description/... 동결본 치환. images v2-actors → imageDomain 반환(아니면 null).
  const { story: resolvedStory, imageDomain } = resolveRelease(story, session?.release_id ?? null);
  story = resolvedStory;
  const imageIndex = getStoryImageIndex(story.id);
  // WS-F P3c: 스토리 전속 로어 + 링크된 전역 로어팩 병합 뷰(팩 엔트리 id 는 'pack-{id}' 문자열).
  const constLore  = getEffectiveConstantLore(story.id);
  const allLore    = getEffectiveAllLore(story.id);
  const activeMsgs = getActiveMessages(sessionId, RECENT_TURNS * 2);

  // ── 공통 준비 ───────────────────────────────────────────────
  // v2-actors cutover: description 에 박힌 legacy 이미지 마크다운(`![](/images/...)`)을 제거.
  // 안 그러면 모델이 카탈로그(/releases/...) 대신 옛 URL 을 계속 배출해 cutover 정합 깨짐(Codex F2).
  let rawDescription = story.description ?? '';
  if (imageDomain) rawDescription = rawDescription.replace(/!\[[^\]]*\]\([^)]*\)/g, '');
  const charSection = [
    rawDescription,
    story.personality ? `\n## 성격\n${story.personality}` : '',
    story.scenario    ? `\n## 시나리오\n${story.scenario}` : '',
  ...
  // composition 은 아래 charNames(L549)에서도 쓰이므로 함수 스코프에서 항상 로드.
  const composition = loadComposition(story.slug);
  let imageSection;
  if (imageDomain) {
    // v2-actors: 동결 actor 카탈로그(release-scoped URL). hasImageMapping 무시(카탈로그가 description 에서 분리됨).
    imageSection = buildActorCatalogText(imageDomain.releaseId, imageDomain.data);
  } else {
    // legacy: 수동 이미지 매핑(외부 URL description 박힘)이면 자동 카탈로그 스킵, 아니면 자동 생성.
    const hasImageMapping = /!\[.*?\]\(https?:\/\//.test(rawDescription);
    imageSection = hasImageMapping ? '' : buildImageSection(story.slug, imageIndex, composition);
  }
  ...
  const embeddedLore = getEffectiveEmbeddedLore(story.id);
