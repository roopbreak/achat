 ...
 *
 * three-part-separation P1: NARRATION_RULES 를 CORE/STATUS/CHOICE 3조각으로 분해.
 * buildNarrationRules({statusMode, choicesMode})가 스토리 설정에 따라 조립한다.
 * 기본값(bottom/on) 조립 결과 = 분해 전 텍스트와 동일 구성(분량 레버 신규 라인 제외).
 */

// ── 서술 코어 (항상 포함) — 입력 형식·절대 규칙·정보 접근·호칭·톤·문장·성적 장면·게이지 ──

const NARRATION_CORE = `## 서술 규칙
...
- **간결성 원칙 비적용 (조기 마감 금지)**: 이 응답은 창작 서술이다. 일반 어시스턴트의 간결성 원칙(짧을수록 좋다)은 적용하지 않는다. 권장 자수 범위를 채우는 것이 간결함보다 우선한다 — 문체의 간결함(짧은 문단·빠른 호흡)은 유지하되, 길이는 비트(대사·리액션·전개)를 더해 채운다.
- **토큰 예산 무시**: 컨텍스트는 서버가 자동 관리한다. 남은 토큰 예산을 이유로 응답을 압축하거나 조기 마감하지 않는다. 분량 지시만 따른다.
- 응답이 권장 자수 미만이면: **일상 구간**은 대사·전개·리액션·다음 비트를 더해 가볍게 채운다(만연체·장황한 분위기 묘사로 늘이지 말 것). **성적 장면**은 감각·자극·몸 반응으로 채운다(아래 성적 장면 보충 순서). 어느 쪽이든 심리 분석·내적 모순 반복으로 늘이지 않는다.
...
// ── 스테이터스 규칙 (status_mode !== 'off' 일 때 포함) ──────────
// 'top' 모드는 D6 보류 — 값만 예약, 현재 bottom 텍스트로 폴백.

const STATUS_RULES_BOTTOM = `### 스테이터스 (매 응답 하단 필수)
캐릭터 설명에 자체 스테이터스 형식이 정의되어 있으면 그 형식을 따른다.
자체 형식이 없으면 아래 기본 형식으로 출력한다.
스테이터스는 서술 본문이 끝난 뒤 반드시 추가한다.

**[필수] 본문↔스테이터스 구분자**: 모든 서술(본문)이 끝나고 스테이터스를 시작하기 직전에, \`⟦STATUS⟧\` 를 **단독 줄로 정확히 한 번** 출력한다. 이 표식은 엔진이 본문과 상태창을 분리하는 기계용 구분자다(사용자에게는 보이지 않는다). 본문 도중에는 절대 넣지 말고, 카드 자체 스테이터스 형식이든 기본 형식이든 **그 직전 줄에 1회만** 넣는다. 형식 예:
(…본문 마지막 문장.)
⟦STATUS⟧
(여기부터 스테이터스 — 카드 형식 또는 기본 형식)
...
// ── 꼬리 구분자만 (status off + choices on — F1: splitChoices 가 status 문자열 기반) ──

const TAIL_SENTINEL_CHOICES_ONLY = `### 응답 꼬리 구분자 (선택지 출력용)
이 스토리는 상태창을 사용하지 않는다. 다만 본문(서술)이 모두 끝나면 \`⟦STATUS⟧\` 를 **단독 줄로 정확히 한 번** 출력한다. 이 표식은 엔진이 본문과 꼬리(선택지)를 분리하는 기계용 구분자다(사용자에게는 보이지 않는다). 본문 도중에는 절대 넣지 않으며, 구분자 아래에 상태창(장소·복장·속마음)을 출력하지 않는다 — 선택지만 출력한다.`;

// ── 선택지 규칙 (choices_mode === 'on' 일 때 포함) ──────────────

function buildChoiceRules(hasStatus) {
  const anchor = hasStatus ? '스테이터스' : '구분자(⟦STATUS⟧)';
  return `### 선택지 (매 응답 마지막 필수)

**출력 게이트 (성적 장면 한정)**: 본문이 3800자 이상이 된 것을 확인한 후, ${anchor} 출력 다음에만 선택지를 출력한다. 본문 미달 시 선택지를 쓰지 말고 본문을 계속 전개한다.

${anchor} 아래에 반드시 2개의 행동 선택지 + 자유 입력을 제시한다.
...
const NSFW_OVERRIDE_SECTION = `## 성적 장면 오버라이드 (NSFW 자극 밀도 우선)
...
/**
 * 스토리 설정에 따라 서술 규칙을 조립한다 (three-part-separation §3-6).
 * 기본값(bottom/on) = 현행 전체 구성. status off 여도 choices on 이면
 * 꼬리 구분자(⟦STATUS⟧)는 유지한다 — 프론트 선택지 추출이 status 문자열 기반(F1).
 *
 * @param {{statusMode?: 'off'|'bottom'|'top', choicesMode?: 'on'|'off'}} [opts]
 */
export function buildNarrationRules({ statusMode = 'bottom', choicesMode = 'on' } = {}) {
  const hasStatus = statusMode !== 'off'; // 'top'은 D6 보류 — bottom 텍스트로 폴백
  const hasChoices = choicesMode !== 'off';
  const parts = [NARRATION_CORE];
  if (hasStatus) parts.push(STATUS_RULES_BOTTOM);
  else if (hasChoices) parts.push(TAIL_SENTINEL_CHOICES_ONLY);
  if (hasChoices) parts.push(buildChoiceRules(hasStatus));
  parts.push(NSFW_OVERRIDE_SECTION);
  return parts.join('\n\n');
}

export const NARRATION_RULES = buildNarrationRules();
...
// ── 분량 체제 (three-part-separation §3-6b — D5) ────────────────
//
// 다이얼 = "분량 목표 밴드"(프롬프트 유도)만 결정. max_tokens 는 전 밴드 공통
// MAX_TOKENS_CAP(사실상 무제한 — 반복 루프 등 폭주 시 재난 캡)으로 고정해
// "목표를 채우면 잘리는" 목표=상한 결합을 제거한다.
// 게이트형 문구: 글자수(모델이 못 세는 단위) + 단락 수(셀 수 있는 단위) 병기,
// 금지형이 아니라 행동형("끝내지 말고 계속 전개")으로. NSFW 3800 게이트(검증됨)와 동일 패턴.
// 성적 장면은 본 가이드를 무시하고 NSFW 오버라이드 자체 하한(3800자)을 따른다.

export const MAX_TOKENS_CAP = 16384;

export const DEFAULT_OUTPUT_BAND = 'full';

export const OUTPUT_TARGETS = {
  short:  '600~900자(2~3단락). 짧고 가볍게, 대사·전개 위주로 핵심 비트만. 본문이 600자 미만이면 응답을 끝내지 말고 대사·리액션을 한 비트 더 전개한 뒤, 진행 중인 비트가 완결되면 자연스럽게 마무리한다.',
  light:  '900~1400자(3~4단락). 대사·리액션·전개로 가볍게 끌고 간다. 본문이 900자(약 3단락) 미만이면 응답을 끝내지 말고 장면을 계속 전개한 뒤, 진행 중인 비트가 완결되면 자연스럽게 마무리한다.',
  medium: '1400~1800자(4~5단락). 빠른 호흡을 유지하며 장면을 전개한다. 본문이 1400자(약 4단락) 미만이면 응답을 끝내지 말고 다음 비트(대사·리액션·전개)를 이어간 뒤, 1400자 이상이 되고 진행 중인 비트가 완결되면 자연스럽게 마무리한다.',
  full:   '1800~2400자(5~7단락). 대사·전개·리액션 위주로 충분히 풀되 만연체로 늘어뜨리지 말 것. 본문이 1800자(약 5단락) 미만이면 응답을 끝내지 말고 장면을 계속 전개한 뒤, 1800자 이상이 되고 진행 중인 비트가 완결되면 자연스럽게 마무리한다.',
  epic:   '2400~3600자(7~10단락). 장면을 끝까지 풀되 가벼운 호흡과 짧은 문단을 유지한다. 본문이 2400자(약 7단락) 미만이면 응답을 끝내지 말고 장면을 계속 전개한 뒤, 2400자 이상이 되고 진행 중인 비트가 완결되면 자연스럽게 마무리한다.',
};

// 밴드별 본문 하한(자) — auto-continue 발동(floor)·디버그 표시 공용. 목표 밴드 하한과 일치.
...
// 구 maxTokens 다이얼 → 밴드 호환 매핑 (기존 클라이언트/저장값 회귀 경로)
export const LEGACY_MAXTOKENS_BAND = {
  1024: 'short',
  2048: 'light',
  3072: 'medium',
  4096: 'full',
...
/** 임의 입력(밴드 키 | 구 maxTokens 숫자 | 그 외)을 유효 밴드로 정규화. */
export function resolveOutputBand(value, fallback = DEFAULT_OUTPUT_BAND) {
  if (value != null && OUTPUT_TARGETS[value]) return value;
  const n = Number(value);
  if (Number.isFinite(n) && LEGACY_MAXTOKENS_BAND[n]) return LEGACY_MAXTOKENS_BAND[n];
  return fallback;
}

/**
 * R2 — 마지막 user 턴 직전 분량 리마인더(depth-0 주입, SillyTavern Author's Note 원리).
 * 프롬프트 전용 — DB 저장 메시지에는 포함하지 않는다.
 */
export function buildOutputReminder(band) {
  const floor = OUTPUT_FLOORS[band] ?? OUTPUT_FLOORS[DEFAULT_OUTPUT_BAND];
  return `(시스템 분량 안내 — 이번 응답 본문은 최소 ${floor}자 이상. 미달이면 끝내지 말고 장면을 계속 전개하고, 채운 뒤 진행 중인 비트가 완결되면 자연스럽게 마무리한다. 성적 장면은 자체 하한(3800자)이 우선.)`;
}

/**
 * R3 — 주인공(플레이어) 침범 금지 리마인더(depth-0). 입력이 능동 모드(`~!`/`~~!`)가
 * 아닐 때 마지막 user 턴에 분량 리마인더와 함께 주입한다. NARRATION_RULES 의
 * "주인공 행동 금지"는 시스템 프롬프트 깊숙이 묻혀 흘려지므로, auto-continue 의
 * buildContinuePrompt 와 동일 취지(주인공 턴 침범 차단)를 1차 생성에도 못 박는다.
 * 누수 사례: `~~`(주인공 수동) 입력에서 NPC가 행동을 요청("앉아")하면 모델이
 * 주인공이 순응하는 모습까지 서술(2026-06-12 원격 재현 확인).
 * 프롬프트 전용 — DB 저장 메시지에는 포함하지 않는다.
 */
export function buildProtagonistGuard() {
  return (
    '(시스템 지시 — 이번 응답에서 주인공(플레이어)의 행동·대사·선택을 새로 ' +
    '생성하지 마세요. NPC가 주인공에게 무언가를 요청·지시해도(예: "앉아", ' +
    '"이리 와", "받아"), 주인공이 그에 응하거나 움직이는 모습까지 서술하지 말고 ' +
    'NPC의 대사·반응·상황 묘사에서 멈춥니다. 주인공이 답할 차례면 그 직전에서 멈춥니다.)'
  );
}
