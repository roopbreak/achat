...
// 자수 권장은 시스템 공통 설정 = "길이 다이얼"(출력량 설정 maxTokens에 매핑).
// 톤은 NARRATION_RULES(작품 톤 — 일상=가벼운 웹소설 / 성적=농밀)가 정의하고, 여기는
// 길이만 조정한다(톤 중립). 일상 구간 기준으로 가볍게 — 성적 장면은 본 가이드를
// 무시하고 NSFW 오버라이드 자체 하한(3800자 등)을 따른다(builtins 성적 장면 규칙 우선).
export const OUTPUT_TARGETS = {
  1024:  '짧고 가볍게. 600~900자. 대사·전개 위주로 핵심 비트만. 늘어뜨리지 말 것.',
  2048:  '900~1400자. 대사·리액션·전개로 가볍게 끌고 간다.',
  3072:  '1400~1800자. 빠른 호흡을 유지하며 장면을 전개한다.',
  4096:  '1800~2400자. 대사·전개·리액션 위주로 충분히 풀되, 만연체로 늘어뜨리지 말 것.',
  8192:  '2400~3600자. 장면을 끝까지 풀되 가벼운 호흡과 짧은 문단을 유지한다.',
};
