...
import { BUILTIN_TEXTS, OUTPUT_TARGETS } from './builtins.mjs';
...
const BUILDERS = {
  builtin_text(block, m) {
    let text = BUILTIN_TEXTS[block.ref];
    if (!text) return null;
    if (block.ref === 'narration_rules') {
      text = text.replace('{{OUTPUT_TARGET}}', m.outputTarget ?? OUTPUT_TARGETS[4096]);
    }
  ...
  mode_overrides(block, m) {
    if (!m.lasciviousOn) return null;
    return `[모드 활성: !음란모드 — 본 가이드는 능동성 가중치만 올린다. 카드별 [Post-History Instructions]가 최종 결정권을 가진다]\n\n${BUILTIN_TEXTS.lascivious_mode}`;
  },
