...
import { buildComposition, loadComposition, saveComposition } from './composition-builder.mjs';
...
export async function autoGenerate(storyName, options = {}) {
  ...
  try {
    // DB에 러닝 잡을 먼저 기록 (락 타이밍 — Codex 리뷰 반영)
    createGenerationJob(jobId, storyName, 100);

    // 1단계: 컴포지션 자동 생성
    console.log(`[AutoGen] ${storyName}: 컴포지션 생성 시작`);
    const composition = await buildComposition(storyName);
    const total = composition.images?.length || 0;

    if (total === 0) {
      throw new Error('컴포지션에 이미지가 없습니다');
    }

    updateGenerationJob(jobId, { total });
