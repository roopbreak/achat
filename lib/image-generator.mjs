  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (qa.passed) {
        ...
        // 기존 파일 삭제 (orphan 방지)
        const oldRow = getDB().prepare('SELECT filename FROM story_images WHERE story_name = ? AND char_dir = ? AND scene_key = ?').get(storyName, charDir, scene.id);
        if (oldRow) {
          const oldPath = charDir
            ? path.join(DATA_DIR, 'stories', storyName, 'images', charDir, oldRow.filename)
            : path.join(DATA_DIR, 'stories', storyName, 'images', oldRow.filename);
...
// ── 좀비 이미지 정리 ────────────────────────────────────
export function cleanupOrphanImages(storyName) {
  const normalized = storyName.normalize('NFC').replace(/[/\\]/g, '');
  const storyDir = path.join(DATA_DIR, 'stories', normalized, 'images');
  ...
  const dbFiles = new Set(getAllStoryImageFilenames(storyName).map(r => {
  ...
  scanDir(storyDir);
  if (deleted.length > 0) console.log(`[Cleanup] ${storyName}: ${deleted.length}개 좀비 이미지 삭제`);
