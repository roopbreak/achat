/**
 * 이미지 배치 생성 + Claude Vision QA + 재생성 오케스트레이터
 * 완전 자동: 스토리 지정 → 100장 생성 → 품질 검증 → DB 등록
 */

import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { generateNAI, buildFullPrompt, buildFullNegative, KARLYN_STYLE } from './nai-client.mjs';
import { loadComposition } from './composition-builder.mjs';
import { callClaudeMultimodal } from './claude-stream.mjs';
import crypto from 'node:crypto';
import {
  insertStoryImage, updateStoryImageMeta, deleteStoryImageBySceneKey,
  createGenerationJob, updateGenerationJob, getRunningJob, getAnyRunningJob,
  getStoryImageCount, getAllStoryImageFilenames, getDB,
  updateJobScene, getPendingJobScenes, getJobSceneCounts, getResumableJobs, bumpJobAttempts,
  getStoryById,
} from './db.mjs';
import { logEvent, logWarn } from './logger.mjs';

/** composition 동결 지문 — resume 시 드리프트 검출(Codex C3) */
export function compositionFingerprint(composition) {
  return crypto.createHash('sha256').update(JSON.stringify(composition)).digest('hex').slice(0, 16);
}

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PROJECT_ROOT = path.resolve(__dirname, '..');
const DATA_DIR = process.env.DATA_DIR ?? path.join(PROJECT_ROOT, 'data');

// ── 생성 큐 (순차 실행) ─────────────────────────────────
const generateQueue = [];
let queueRunning = false;

const MAX_QUEUE = 100;

async function processQueue() {
  if (queueRunning) return;
  queueRunning = true;
  try {
    while (generateQueue.length > 0) {
      const { resolve, reject, fn } = generateQueue.shift();
      try { resolve(await fn()); }
      catch (e) { reject(e); }
    }
  } finally { queueRunning = false; }
}

export function enqueueGenerate(fn) {
  return new Promise((resolve, reject) => {
    if (generateQueue.length >= MAX_QUEUE) return reject(new Error('생성 큐가 가득 찼습니다 (최대 100)'));
    generateQueue.push({ resolve, reject, fn });
    processQueue();
  });
}

export function getQueueLength() {
  return generateQueue.length + (queueRunning ? 1 : 0);
}

export function clearQueue() {
  const cleared = generateQueue.length;
  generateQueue.forEach(({ reject }) => reject(new Error('큐 중지됨')));
  generateQueue.length = 0;
  return cleared;
}

// ── 의존성 검증 ──────────────────────────────────────────
export function checkDependencies() {
  const issues = [];
  if (!process.env.NAI_API_TOKEN) issues.push('NAI_API_TOKEN 환경변수 없음');
  if (!process.env.ANTHROPIC_API_KEY) issues.push('ANTHROPIC_API_KEY 환경변수 없음');
  return issues;
}

// ── 이미지 저장 헬퍼 ─────────────────────────────────────
function saveGeneratedImage(slug, charDir, filename, buffer) {
  const dir = charDir
    ? path.join(DATA_DIR, 'stories', slug, 'images', charDir)
    : path.join(DATA_DIR, 'stories', slug, 'images');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, filename), buffer);
}

// ── 배열 청크 분할 ───────────────────────────────────────
function chunk(arr, size) {
  const chunks = [];
  for (let i = 0; i < arr.length; i += size) {
    chunks.push(arr.slice(i, i + size));
  }
  return chunks;
}

// ── Claude Vision QA ─────────────────────────────────────
async function validateImage(imageBuffer) {
  try {
    const base64 = imageBuffer.toString('base64');
    const raw = await callClaudeMultimodal({
      model: 'claude-haiku-4-5-20251001',
      messages: [{
        role: 'user',
        content: [
          {
            type: 'image',
            source: { type: 'base64', media_type: 'image/png', data: base64 },
          },
          {
            type: 'text',
            text: `이미지에 여자가 몇 명 있는지 세어주세요.
또한 심각한 해부학적 오류(팔 3개, 머리 2개 등)가 있는지 확인하세요.

출력 JSON만: { "female_count": number, "severe_defect": boolean, "passed": boolean }
- female_count가 2 이상이면 passed=false
- severe_defect가 true이면 passed=false
- 그 외에는 passed=true (경미한 차이는 합격)`,
          },
        ],
      }],
      maxTokens: 256,
    });

    const jsonStr = raw.replace(/^```json?\s*\n?/m, '').replace(/\n?```\s*$/m, '').trim();
    const result = JSON.parse(jsonStr);
    // 로컬 규칙 재계산 — Claude 반환 passed를 맹신하지 않음
    const femaleCount = result.female_count ?? 1;
    const severeDefect = result.severe_defect ?? false;
    const passed = femaleCount === 1 && !severeDefect;
    return { passed, female_count: femaleCount, severe_defect: severeDefect };
  } catch (e) {
    console.warn('[QA] Vision 검증 실패, 불합격 처리:', e.message);
    return { passed: false, female_count: -1, severe_defect: false };
  }
}

// ── 단일 장면 생성 + QA ──────────────────────────────────
async function generateWithQA(story, composition, scene, maxRetries) {
  const character = composition.characters[scene.character || 'main'];
  if (!character) throw new Error(`캐릭터 없음: ${scene.character}`);

  let lastBuffer, lastSeed;
  let qaRetries = 0;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    // 프롬프트 조합 (2-Layer)
    const variationParts = [];
    const framing = scene.framing
      || composition.defaults?.category_framing?.[scene.category]
      || '';
    if (framing) variationParts.push(framing);
    if (scene.outfit) variationParts.push(scene.outfit);
    if (scene.pose) variationParts.push(scene.pose);
    if (scene.expression) variationParts.push(scene.expression);
    if (scene.custom_tags) variationParts.push(scene.custom_tags);
    const variation = variationParts.filter(Boolean).join(', ');

    const prompt = buildFullPrompt(character.base_prompt, variation);
    let negative = buildFullNegative(character.base_negative, scene.custom_negative);

    // 카테고리 네거티브 (사극 기모노 혼동 방지 등) 추가
    const categoryNeg = composition.defaults?.category_negative;
    if (categoryNeg) {
      negative = negative ? `${negative}, ${categoryNeg}` : categoryNeg;
    }

    // adult 장면에서 1boy가 pose에 있으면 네거티브에서 남성 관련 태그 제거
    if (scene.pose?.includes('1boy')) {
      negative = negative.replace(/\b1boy\b,?\s*/g, '').replace(/\bmultiple boys\b,?\s*/g, '').replace(/\bmale focus\b,?\s*/g, '');
    }

    // 재시도 시 네거티브 강화
    if (attempt > 0) {
      negative += ', ((2girls)), ((multiple girls)), ((clone)), ((split screen))';
    }

    const aspectRatio = scene.aspect_ratio
      || composition.defaults?.category_aspect_ratios?.[scene.category]
      || composition.defaults?.aspect_ratio
      || '3:4';

    try {
      const { buffer, seed } = await generateNAI(process.env.NAI_API_TOKEN, {
        prompt,
        negativePrompt: negative,
        aspectRatio,
        model: KARLYN_STYLE.params.model,
        steps: KARLYN_STYLE.params.steps,
        scale: KARLYN_STYLE.params.scale,
        rescale: KARLYN_STYLE.params.rescale,
        sampler: KARLYN_STYLE.params.sampler,
      });

      lastBuffer = buffer;
      lastSeed = seed;

      // QA 검증
      const qa = await validateImage(buffer);

      if (qa.passed) {
        const charDir = Object.keys(composition.characters).length > 1 ? (scene.character || 'main') : '';
        // 기존 파일 삭제 (orphan 방지)
        const oldRow = getDB().prepare('SELECT filename FROM story_images WHERE story_id = ? AND char_dir = ? AND scene_key = ?').get(story.id, charDir, scene.id);
        if (oldRow) {
          const oldPath = charDir
            ? path.join(DATA_DIR, 'stories', story.slug, 'images', charDir, oldRow.filename)
            : path.join(DATA_DIR, 'stories', story.slug, 'images', oldRow.filename);
          try { fs.unlinkSync(oldPath); } catch {}
        }
        deleteStoryImageBySceneKey(story.id, charDir, scene.id);

        const filename = `batch_${scene.id}_${Date.now()}.png`;
        saveGeneratedImage(story.slug, charDir, filename, buffer);
        insertStoryImage(story.id, charDir, scene.id, filename);
        updateStoryImageMeta(story.id, filename, { prompt, seed, source: 'batch' });

        return { scene: scene.id, filename, status: 'passed', attempt, qaRetries };
      }

      qaRetries++;
      console.log(`[QA] ${scene.id} 불합격 (여자 ${qa.female_count}명, 결함: ${qa.severe_defect}), 재시도 ${attempt + 1}/${maxRetries + 1}`);

    } catch (e) {
      console.error(`[Gen] ${scene.id} 생성 실패:`, e.message);
      if (attempt === maxRetries) {
        return { scene: scene.id, status: 'failed', reason: e.message, qaRetries };
      }
    }
  }

  // 최대 재시도 후 불합격 — 파일 저장하지 않음 (좀비 이미지 방지)
  console.log(`[QA] ${scene.id} 최종 불합격 → 폐기`);
  return { scene: scene.id, status: 'failed', reason: 'QA 재시도 초과', qaRetries };
}

// ── QA 학습 기록 ─────────────────────────────────────────
function recordLearnings(failures) {
  const learningsPath = path.join(PROJECT_ROOT, 'docs', 'rag', 'learnings', 'tag-patterns.md');
  try {
    fs.mkdirSync(path.dirname(learningsPath), { recursive: true });
    const existing = fs.existsSync(learningsPath) ? fs.readFileSync(learningsPath, 'utf-8') : '# 검증된 태그 패턴\n\n';
    const newEntries = failures.map(f =>
      `- ${new Date().toISOString().slice(0, 10)}: ${f.scene} — ${f.reason || 'QA 불합격'}`
    ).join('\n');
    fs.writeFileSync(learningsPath, existing + '\n' + newEntries + '\n');
  } catch (e) {
    console.warn('[Learning] 학습 기록 실패:', e.message);
  }
}

// ── 좀비 이미지 정리 ────────────────────────────────────
const SLUG_RE_INTERNAL = /^[a-z0-9][a-z0-9-]{2,49}$/;

export function cleanupOrphanImages(story) {
  // slug 강한 검증 — DB값이라도 traversal 차단
  if (!story?.slug || !SLUG_RE_INTERNAL.test(story.slug)) {
    throw new Error(`cleanupOrphanImages: 잘못된 slug "${story?.slug}"`);
  }
  const slug = story.slug;
  const storyDir = path.join(DATA_DIR, 'stories', slug, 'images');
  const realStoryDir = fs.existsSync(storyDir) ? fs.realpathSync(storyDir) : null;
  const storiesBase = fs.realpathSync(path.join(DATA_DIR, 'stories'));
  if (!realStoryDir || !realStoryDir.startsWith(storiesBase)) return { deleted: 0, files: [] };

  const dbFiles = new Set(getAllStoryImageFilenames(story.id).map(r => {
    return r.charDir ? path.join(r.charDir, r.filename) : r.filename;
  }));

  const deleted = [];

  function scanDir(dir, prefix = '') {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      if (entry.isDirectory()) {
        scanDir(path.join(dir, entry.name), prefix ? `${prefix}/${entry.name}` : entry.name);
      } else if (entry.name.endsWith('.png')) {
        const relPath = prefix ? `${prefix}/${entry.name}` : entry.name;
        const dbKey = prefix ? path.join(prefix, entry.name) : entry.name;
        if (!dbFiles.has(dbKey)) {
          try { fs.unlinkSync(path.join(dir, entry.name)); deleted.push(relPath); }
          catch (e) { console.warn(`[Cleanup] 삭제 실패: ${relPath}:`, e.message); }
        }
      }
    }
  }

  scanDir(storyDir);
  if (deleted.length > 0) console.log(`[Cleanup] ${story.slug}: ${deleted.length}개 좀비 이미지 삭제`);
  return { deleted: deleted.length, files: deleted };
}

// ── 메인: 완전 자동 생성 ─────────────────────────────────
export async function autoGenerate(story, options = {}) {
  const {
    concurrency = 1,
    maxRetries = 2,
    onProgress = null,
    sceneIds = null,
    jobId: existingJobId = null,   // P5b resume: 기존 job 이어받기
  } = options;

  // 의존성 검증
  const issues = checkDependencies();
  if (issues.length > 0) {
    throw new Error(`의존성 부족: ${issues.join(', ')}`);
  }

  // composition 로드 (없으면 에러)
  const composition = loadComposition(story.slug);
  if (!composition) {
    throw new Error('컴포지션이 없습니다. 먼저 컴포지션을 생성하세요.');
  }
  const chars = composition.characters || {};
  const emptyChars = Object.entries(chars).filter(([, c]) => !c.base_prompt).map(([k]) => k);
  if (emptyChars.length > 0) {
    throw new Error(`base_prompt가 비어있는 캐릭터: ${emptyChars.join(', ')}. 컴포지션에 외모 태그를 설정하세요.`);
  }

  // 생성 대상 장면 필터링
  let scenes = composition.images || [];
  if (sceneIds && sceneIds.length > 0) {
    scenes = scenes.filter(s => sceneIds.includes(s.id));
    if (scenes.length === 0) {
      throw new Error(`지정된 장면을 찾을 수 없습니다: ${sceneIds.join(', ')}`);
    }
  }

  const total = scenes.length;
  if (total === 0) {
    throw new Error('컴포지션에 이미지가 없습니다');
  }

  const jobId = existingJobId ?? `gen_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

  try {
    if (existingJobId) {
      // resume: job 행은 이미 존재 — running 전환 + 실행 시작 시점에 attempts 소모
      // (재큐 예약만으로 한도를 깎지 않음 — Codex P5b M3)
      updateGenerationJob(jobId, { status: 'running' });
      bumpJobAttempts(jobId);
    } else {
      createGenerationJob(jobId, story.id, total, {
        slug: story.slug,
        sceneIds: scenes.map(sc => sc.id),
        composition_fingerprint: compositionFingerprint(composition),
      }, scenes.map(sc => sc.id));
    }
    logEvent('gen_job', { jobId, slug: story.slug, status: 'running', total, resume: Boolean(existingJobId) });
    onProgress?.({ jobId, status: 'running', completed: 0, total, failed: 0 });

    console.log(`[AutoGen] ${story.slug}: ${total}장 생성 시작 (병렬 ${concurrency})`);
    const results = [];
    let totalQaRetries = 0;

    for (const batch of chunk(scenes, concurrency)) {
      const batchResults = await Promise.all(
        batch.map(scene => generateWithQA(story, composition, scene, maxRetries))
      );
      results.push(...batchResults);
      totalQaRetries += batchResults.reduce((sum, r) => sum + (r.qaRetries || 0), 0);

      // scene 단위 상태 기록 — resume 시 완료 scene 재생성 방지(P5b)
      for (const r of batchResults) {
        updateJobScene(jobId, r.scene, r.status === 'passed' ? 'done' : 'error', r.reason ?? null);
      }

      const counts = getJobSceneCounts(jobId);
      const completed = counts?.done ?? results.filter(r => r.status === 'passed').length;
      const failed = counts?.error ?? results.filter(r => r.status === 'failed').length;

      updateGenerationJob(jobId, { completed, failed, qa_retries: totalQaRetries });
      onProgress?.({ jobId, status: 'running', completed, total, failed });
    }

    // 3단계: QA 학습 기록
    const failures = results.filter(r => r.status === 'failed');
    if (failures.length > 0) {
      recordLearnings(failures);
    }

    const finalCounts = getJobSceneCounts(jobId);
    const passed = finalCounts?.done ?? results.filter(r => r.status === 'passed').length;
    const failedTotal = finalCounts?.error ?? failures.length;
    updateGenerationJob(jobId, {
      status: 'completed',
      completed: passed,
      failed: failedTotal,
      qa_retries: totalQaRetries,
      finished_at: new Date().toISOString(),
    });
    logEvent('gen_job', { jobId, slug: story.slug, status: 'completed', passed, failed: failedTotal });

    console.log(`[AutoGen] ${story.slug}: 완료 — ${passed}/${total} 통과, ${failures.length} 실패, QA 재시도 ${totalQaRetries}회`);
    onProgress?.({ jobId, status: 'completed', completed: passed, total, failed: failures.length });

    return { jobId, total, passed, failed: failures.length, qaRetries: totalQaRetries, results };

  } catch (e) {
    try {
      updateGenerationJob(jobId, { status: 'failed', error: e.message, finished_at: new Date().toISOString() });
    } catch {}
    onProgress?.({ jobId, status: 'failed', error: e.message });
    throw e;

  }
}


// ── P5b: 부팅 시 미완료 job resume (plan §3.3 — Codex C2·C3) ──────────────
const RESUME_MAX_ATTEMPTS = 2;

export function resumeGenerationJobs() {
  let jobs;
  try { jobs = getResumableJobs(RESUME_MAX_ATTEMPTS); }
  catch (e) { logWarn('gen_resume_error', { message: e.message }); return { resumed: 0 }; }
  let resumed = 0;
  for (const job of jobs) {
    try {
      const payload = job.payload ? JSON.parse(job.payload) : null;
      if (!payload?.slug) {
        // 구(008 이전) job — payload 없음: resume 불가, 정직하게 실패 마킹
        updateGenerationJob(job.id, { status: 'failed', error: '서버 재시작(레거시 job — payload 없음)', finished_at: new Date().toISOString() });
        continue;
      }
      const story = getStoryById(job.story_id);
      if (!story) { updateGenerationJob(job.id, { status: 'failed', error: '스토리 없음', finished_at: new Date().toISOString() }); continue; }
      const composition = loadComposition(payload.slug);
      if (!composition || compositionFingerprint(composition) !== payload.composition_fingerprint) {
        // 컴포지션 드리프트 — 원래 job 과 다른 결과가 나오므로 재개하지 않음(운영자 재발행)
        updateGenerationJob(job.id, { status: 'stale', error: 'composition 변경 감지 — resume 중단', finished_at: new Date().toISOString() });
        logWarn('gen_job', { jobId: job.id, slug: payload.slug, status: 'stale' });
        continue;
      }
      const pending = getPendingJobScenes(job.id);
      if (pending.length === 0) {
        const counts = getJobSceneCounts(job.id);
        updateGenerationJob(job.id, { status: 'completed', completed: counts?.done ?? 0, failed: counts?.error ?? 0, finished_at: new Date().toISOString() });
        continue;
      }
      logEvent('gen_job', { jobId: job.id, slug: payload.slug, status: 'resume', pending: pending.length, attempts: job.attempts });
      enqueueGenerate(() => autoGenerate(story, { sceneIds: pending, jobId: job.id }))
        .catch(err => logWarn('gen_job', { jobId: job.id, status: 'resume-failed', message: err.message }));
      resumed++;
    } catch (e) {
      logWarn('gen_resume_error', { jobId: job.id, message: e.message });
    }
  }
  if (jobs.length) logEvent('gen_resume', { candidates: jobs.length, resumed });
  return { resumed };
}
