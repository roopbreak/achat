...
function normalizeCustomScene(scene, category, index, charKey) {
  const id = scene.id || `${category}-custom-${String(index + 1).padStart(2, '0')}`;
  return {
    character: charKey,
    category,
...
// customScenes: { daily: [...], outfit: [...], location: [...], special: [...], interaction: [...] }
//   - 싱글 캐릭터에만 적용. 멀티 캐릭터는 기존 fallback 템플릿 그대로
...
export function buildComposition(storyName, opts = {}) {
  ...
  // 커스텀 씬은 싱글 캐릭터에만 적용
  const customScenes = (!isMulti && opts.customScenes) ? opts.customScenes : null;
  ...
  if (isMulti) {
    ...
    // 싱글 캐릭터: 코어(expression/adult) + interaction 코어 5장 + 커스텀(daily/outfit/location/special/interaction-custom)
    const charKey = charKeys[0];
    ...
    for (const [category, scenes] of Object.entries(templates)) {
      ...
      // 1. 코어 카테고리(expression/adult): 항상 자동 템플릿 사용
      if (isCore) {
      ...
      // 2. interaction: customScenes 모드면 코어 5장 + customScenes.interaction, 아니면 fallback 전체
      if (isInteraction) {
        if (customScenes) {
          // 코어 5장 (보편 인터랙션)
          for (const scene of scenes) {
            if (CORE_INTERACTION_IDS.has(scene.id)) {
              images.push({
                character: charKey,
                category,
                framing: CATEGORY_FRAMING[category],
          ...
          // 맞춤 인터랙션 (composition-designer 작성, 있으면)
          const customInter = customScenes.interaction;
          if (Array.isArray(customInter)) {
            customInter.forEach((scene, idx) => {
              images.push(normalizeCustomScene(scene, category, idx, charKey));
            });
          }
        } else {
          // 하위 호환: customScenes 모드가 아니면 기존 전체 interaction 템플릿
          for (const scene of scenes) {
            images.push({
              character: charKey,
              category,
      ...
      // 3. 커스텀 카테고리(daily/outfit/location/special): customScenes 있으면 대체, 없으면 fallback
      if (isCustom) {
        const custom = customScenes?.[category];
  ...
  const customInfo = customScenes
    ? ` (커스텀: ${Object.entries(customScenes).filter(([, v]) => Array.isArray(v) && v.length > 0).map(([k, v]) => `${k}=${v.length}`).join(', ') || '없음'})`
    : '';
  console.log(`[Composition] ${storyName}: 템플릿 생성 완료 (${charKeys.length}캐릭터, ${images.length}장, 타입=${templateType}${customInfo})`);
  return composition;
}
