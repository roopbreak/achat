 ...
 * RAG 검색 + Claude API로 100장 장면 목록 설계
 */
...
// ── RAG 검색 ─────────────────────────────────────────────
function searchRAG(query, top = 5) {
  try {
    const scriptPath = path.join(PROJECT_ROOT, 'scripts', 'rag-search.py');
    const result = execSync(
      `python3 "${scriptPath}" "${query.replace(/"/g, '\\"')}" --top ${top}`,
      { encoding: 'utf-8', timeout: 30000, cwd: PROJECT_ROOT }
    );
    return result.trim();
  } catch (e) {
    console.warn(`[RAG] 검색 실패 (${query}):`, e.message);
    return '';
  }
}
...
export async function buildComposition(storyName) {
  ...
  // 1단계: RAG 검색
  console.log(`[Composition] ${storyName}: RAG 검색 시작`);
  const ragContext = {
    appearance: searchRAG(`${story.description?.slice(0, 100)} danbooru 태그 변환 외모`),
    expression: searchRAG('표정 태그 레시피 감정 표현 기쁨 슬픔 놀람'),
    outfit: searchRAG('의상 태그 종류 복장 교복 사복 정장'),
    pose: searchRAG('포즈 구도 카메라 앵글 cowboy shot'),
    nsfw: searchRAG('체위 태그 정상위 후배위 기승위 missionary doggystyle'),
    negative: searchRAG('네거티브 프롬프트 2girls 방지 multiple girls'),
    framing: searchRAG('프레이밍 close-up face focus head shot upper body'),
  };
  console.log(`[Composition] ${storyName}: RAG 검색 완료`);

  // 2단계: Claude API로 composition 생성
  const systemPrompt = `당신은 NovelAI 이미지 생성 전문가입니다. 반드시 유효한 JSON만 출력하세요. 설명이나 마크다운 코드블록 없이 순수 JSON만 출력합니다.`;

  const userPrompt = buildClaudePrompt(story, lores, ragContext);
  ...
  composition._rag_log = Object.entries(ragContext).map(([k, v]) => ({
    query: k, applied: v ? `${v.split('---').length - 1}건 참조` : '해당 없음',
  }));
...
// ── Claude 프롬프트 빌더 ─────────────────────────────────
function buildClaudePrompt(story, lores, ragContext) {
...
== 외모 태그 변환 ==
${ragContext.appearance || '(없음)'}

== 표정 태그 레시피 ==
${ragContext.expression || '(없음)'}

== 의상 태그 ==
${ragContext.outfit || '(없음)'}
...
== 네거티브 프롬프트 패턴 ==
${ragContext.negative || '(없음)'}

== 프레이밍 ==
${ragContext.framing || '(없음)'}
...
2. characters.main.base_prompt: Seed 고정부 (외모+글래머 체형, RAG 검증 태그 사용)
