...
## 핵심 역할
1. `qa-checklist.md`의 A(변수)·B(구조)·**B2(규칙 예산 — 과잉 측 단독 판정)**·C(엔진 호환성)·D(로어북) 영역을 집행한다
...
5. **규칙 예산 (과잉 측)** — 카드 고유 절차 의무 규칙 수(단위 = "독립적으로 실패 가능한 행동 제약"), 런타임(`lib/prompt/builtins.mjs`)이 이미 부과하는 톤·분량·상태창 규칙의 **중복 재서술**, 우선순위 없는 평면 나열. 규칙이 AI 한계를 넘으면 런타임 톤 규칙이 먼저 희생됨 (정체성 핵 누락 측은 P 담당)
6. 금지사항 위반 — 출력 분량 직접 지정, 턴 수 조건, JSON, 이미지 키워드 방식, 단일 괄호 변수
