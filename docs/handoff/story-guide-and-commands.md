...
> 상태: 활성 (Phase 1·2 완료, Phase 3·4 대기) | 마지막 업데이트: 2026-05-14

## 현재 상태

**Phase 1·2 구현·배포·원격 검증 완료** (커밋 `64fc286`).
Phase 3·4 = 원격 스토리 전수 점검·`!` 통일·정규화는 **별도 사이클로 대기 중**.

- 백엔드: `commands` 컬럼 마이그레이션·헬퍼·엔드포인트 — 원격 검증 완료
- 프론트엔드: 상세 페이지·가이드 패널·커맨드 편집 UI — 원격 검증 완료
- Codex 리뷰 2회(플랜 + 코드) 반영 완료
- 원격 검증(`risu.ddsmdy.com`): `/api/stories`·`/api/stories/:name`·`/recent`·404 정상, StoryDetail 페이지 렌더·빈 커맨드 fallback 확인

## 다음 작업 (Phase 3·4 — 별도 사이클)

플랜 문서의 Phase 3·4 섹션 참조. 요약:
- Phase 3: `scripts/audit-commands.mjs` 신설 — 원격 모든 스토리 스캔, `!커맨드` 추출 + 비-`!`/불일치 리포트
- Phase 4: 검토 후 스냅샷·복구 payload 동반하여 `commands` 필드 채우기 + `!` 통일 반영
