// 원격 서버에서 실행 — male-conservation-clinic 검수 반영
// /tmp/clinic-payload-story.json, /tmp/clinic-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/clinic-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: set -a; source .env; node clinic-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '어서오세요 남성 보존 클리닉에';
const NAME_ENC = encodeURIComponent(NAME);

const story = JSON.parse(fs.readFileSync('/tmp/clinic-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/clinic-payload-lore-plan.json', 'utf8'));
...
function fin() {
  ...
  fs.writeFileSync('/tmp/clinic-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 적용 결과 ===');
