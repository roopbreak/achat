// 원격 서버에서 실행 — male-conservation-clinic 2차 검수 반영
// /tmp/clinic-v2-payload-story.json, /tmp/clinic-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 2차는 creates·deletes 0 — PUT story + PUT updates 11만.
// 결과: /tmp/clinic-v2-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: set -a; source .env; node clinic-v2-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '어서오세요 남성 보존 클리닉에';
const NAME_ENC = encodeURIComponent(NAME);

const story = JSON.parse(fs.readFileSync('/tmp/clinic-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/clinic-v2-payload-lore-plan.json', 'utf8'));
...
async function main() {
  ...
  for (const u of plan.updates) {
    try {
      ...
      log.updated.push({ id, name: u.name ?? `(id ${id})` });
      console.log(`[3/4] update [${id}] "${u.name ?? id}" — OK`);
...
function fin() {
  ...
  fs.writeFileSync('/tmp/clinic-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
