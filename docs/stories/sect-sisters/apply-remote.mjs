// 원격 서버에서 실행 — sect-sisters (섹트여동생 벗방누나) 검수 반영
// /tmp/eunseo-payload-story.json, /tmp/eunseo-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/eunseo-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: APP_SECRET=xxx node eunseo-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '섹트여동생 벗방누나';
const ENC = encodeURIComponent(NAME);

const story = JSON.parse(fs.readFileSync('/tmp/eunseo-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/eunseo-payload-lore-plan.json', 'utf8'));
...
function fin() {
  ...
  fs.writeFileSync('/tmp/eunseo-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 적용 결과 ===');
