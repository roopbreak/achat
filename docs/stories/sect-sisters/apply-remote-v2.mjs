// 원격 서버에서 실행 — sect-sisters (섹트여동생 벗방누나) 2차 풀-리라이트 반영
// /tmp/eunseo-v2-payload-story.json, /tmp/eunseo-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(2차는 0건)
// 결과: /tmp/eunseo-v2-apply-log.json
import fs from 'fs';
...
const story = JSON.parse(fs.readFileSync('/tmp/eunseo-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/eunseo-v2-payload-lore-plan.json', 'utf8'));
...
function fin() {
  ...
  fs.writeFileSync('/tmp/eunseo-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
