// 원격 서버에서 실행 — sieun-smartphone ("여사친의 스마트폰") 2차 풀-리라이트 반영
// /tmp/sieun-v2-payload-story.json, /tmp/sieun-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/sieun-v2-apply-log.json
import fs from 'fs';
...
const story = JSON.parse(fs.readFileSync('/tmp/sieun-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/sieun-v2-payload-lore-plan.json', 'utf8'));
...
async function main() {
  ...
  // 4. DELETE deletes (파괴적 — 마지막). 2차는 deleteIds 비어 있음
...
function fin() {
  ...
  fs.writeFileSync('/tmp/sieun-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
