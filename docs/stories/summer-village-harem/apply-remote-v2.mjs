// 원격 서버에서 실행 — summer-village-harem (에어컨 없는 여름, 시골, X스) 2차 풀-리라이트 반영
// /tmp/choi-v2-payload-story.json, /tmp/choi-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/choi-v2-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: set -a; source .env; node choi-v2-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '에어컨 없는 여름, 시골, X스';
const ENC = encodeURIComponent(NAME);

const story = JSON.parse(fs.readFileSync('/tmp/choi-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/choi-v2-payload-lore-plan.json', 'utf8'));
...
async function main() {
  ...
  // 4. DELETE deletes (파괴적 — 마지막). 2차는 deleteIds 0개라 보통 스킵.
...
function fin() {
  ...
  fs.writeFileSync('/tmp/choi-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
