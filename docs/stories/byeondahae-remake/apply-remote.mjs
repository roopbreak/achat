// 원격 서버에서 실행 — 변다해 (리메이크) 검수 반영
// /tmp/byeondahae-payload-story.json, /tmp/byeondahae-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/byeondahae-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: set -a; source .env; node byeondahae-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '변다해 (리메이크)';
const ENC = encodeURIComponent(NAME);

const story = JSON.parse(fs.readFileSync('/tmp/byeondahae-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/byeondahae-payload-lore-plan.json', 'utf8'));
...
function fin() {
  ...
  fs.writeFileSync('/tmp/byeondahae-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 적용 결과 ===');
