// 원격 서버에서 실행 — 변다해 (리메이크) 2차 풀-리라이트 반영
// /tmp/byeondahae-v2-payload-story.json, /tmp/byeondahae-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/byeondahae-v2-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: set -a; source .env; node byeondahae-v2-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '변다해 (리메이크)';
// 한글/괄호 — encodeURIComponent + 괄호도 %28 %29
const ENC = encodeURIComponent(NAME).replace(/\(/g, '%28').replace(/\)/g, '%29');

const story = JSON.parse(fs.readFileSync('/tmp/byeondahae-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/byeondahae-v2-payload-lore-plan.json', 'utf8'));

const log = { startedAt: new Date().toISOString(), storyName: NAME, story: null, created: [], updated: [], deleted: [], errors: [] };
...
async function main() {
  ...
  // 2. POST creates (신규 — 안전, 추가만)
  for (const c of plan.creates) {
    try {
      const r = await call('POST', `${BASE}/${ENC}/lore`, c);
  ...
  for (const u of plan.updates) {
    try {
      const { id, ...body } = u;
      await call('PUT', `${BASE}/${ENC}/lore/${id}`, body);
  ...
  // 4. DELETE deletes (파괴적 — 마지막). 이번 2차는 삭제 0건.
  for (const id of plan.deleteIds) {
    try {
      await call('DELETE', `${BASE}/${ENC}/lore/${id}`);
      log.deleted.push(id);
...
function fin() {
  ...
  fs.writeFileSync('/tmp/byeondahae-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
