// 원격 서버에서 실행 — bangkok-poolvilla 검수 반영
// /tmp/bp-payload-story.json, /tmp/bp-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/bp-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: APP_SECRET=xxx node bp-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = 'bangkok-poolvilla';

const story = JSON.parse(fs.readFileSync('/tmp/bp-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/bp-payload-lore-plan.json', 'utf8'));
...
async function main() {
  ...
  try {
    await call('PUT', `${BASE}/${NAME}`, story);
  ...
  for (const c of plan.creates) {
    try {
      const r = await call('POST', `${BASE}/${NAME}/lore`, c);
  ...
  for (const u of plan.updates) {
    try {
      const { id, ...body } = u;
      await call('PUT', `${BASE}/${NAME}/lore/${id}`, body);
  ...
  for (const id of plan.deleteIds) {
    try {
      await call('DELETE', `${BASE}/${NAME}/lore/${id}`);
      log.deleted.push(id);
...
function fin() {
  ...
  fs.writeFileSync('/tmp/bp-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 적용 결과 ===');
