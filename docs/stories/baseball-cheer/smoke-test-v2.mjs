// 원격에서 실행 — 야구나 잘하라고 2차 스모크 테스트 (2턴)
// localhost:8080 chat SSE 소비. 결과 /tmp/yuna-v2-smoke-log.json
import fs from 'fs';
const NAME = '야구나 잘하라고';
const BASE = 'http://localhost:8080/api/stories/' + encodeURIComponent(NAME) + '/chat';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: APP_SECRET=xxx node yuna-v2-smoke-test.mjs'); process.exit(1); }
...
function analyze(label, r) {
  ...
  return {
    ...
    user선점잔존: /\{\{user\}\}\s*\|/.test(t) ? '❌ {{user}} 발화 선점' : '✅ 없음',
...
async function main() {
  ...
  try {
    // Turn 1 — 구단주 모드 선택. first_mes는 모드 선택 UI 전용이므로
    // AI가 모드 선택 직후 구단주 모드 도입 장면을 새로 생성해야 함.
    const t1 = await turn('!구단주모드', undefined);
    const a1 = analyze('T1 (!구단주모드 — 모드 도입 생성 기대)', t1);
    log.turns.push(a1);
    console.log('--- T1 ---'); console.log(JSON.stringify(a1, null, 2));

    if (!a1.sessionId) throw new Error('T1에서 sessionId 미회수 — 중단');

    // Turn 2 — 차유리 트리거 + 경기 결과 복합 트리거 + 이미지 형식 재확인
    const t2 = await turn('유리에게 시선을 돌린다. "유리. 오늘 경기 결과는 어떻게 됐지? 동규 타석 성적 말해봐."', a1.sessionId);
    const a2 = analyze('T2 (차유리 + 경기 결과 복합 트리거)', t2);
    log.turns.push(a2);
  ...
  fs.writeFileSync('/tmp/yuna-v2-smoke-log.json', JSON.stringify(log, null, 2));
}
main();
