// 원격 서버에서 실행 — momo-surrogate(모모) 검수 반영
// /tmp/momo-payload-story.json, /tmp/momo-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/momo-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: set -a; source .env; node momo-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '모모';
const NAME_ENC = encodeURIComponent(NAME);

const story = JSON.parse(fs.readFileSync('/tmp/momo-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/momo-payload-lore-plan.json', 'utf8'));
...
function fin() {
  ...
  fs.writeFileSync('/tmp/momo-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 적용 결과 ===');
