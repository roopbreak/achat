// 원격 서버에서 실행 — dayoung-piled-up (너 쌓여있잖아) 검수 반영
// /tmp/dayoung-payload-story.json, /tmp/dayoung-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/dayoung-apply-log.json
import fs from 'fs';

const BASE = 'http://localhost:8080/api/admin/stories';
const SECRET = process.env.APP_SECRET;
if (!SECRET) { console.error('APP_SECRET 환경변수 필요 — 예: APP_SECRET=xxx node dayoung-apply-remote.mjs'); process.exit(1); }
const AUTH = 'Bearer ' + SECRET;
const NAME = '너 쌓여있잖아';

const story = JSON.parse(fs.readFileSync('/tmp/dayoung-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/dayoung-payload-lore-plan.json', 'utf8'));
...
async function main() {
  ...
  try {
    await call('PUT', `${BASE}/${encodeURIComponent(NAME)}`, story);
  ...
  for (const c of plan.creates) {
    try {
      const r = await call('POST', `${BASE}/${encodeURIComponent(NAME)}/lore`, c);
  ...
  for (const u of plan.updates) {
    try {
      const { id, ...body } = u;
      await call('PUT', `${BASE}/${encodeURIComponent(NAME)}/lore/${id}`, body);
  ...
  for (const id of plan.deleteIds) {
    try {
      await call('DELETE', `${BASE}/${encodeURIComponent(NAME)}/lore/${id}`);
      log.deleted.push(id);
...
function fin() {
  ...
  fs.writeFileSync('/tmp/dayoung-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 적용 결과 ===');
