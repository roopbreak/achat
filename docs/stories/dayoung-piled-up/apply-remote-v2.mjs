// 원격 서버에서 실행 — dayoung-piled-up (너 쌓여있잖아) 2차 풀-리라이트 반영
// /tmp/dayoung-v2-payload-story.json, /tmp/dayoung-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 결과: /tmp/dayoung-v2-apply-log.json
import fs from 'fs';
...
const story = JSON.parse(fs.readFileSync('/tmp/dayoung-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/dayoung-v2-payload-lore-plan.json', 'utf8'));
...
function fin() {
  ...
  fs.writeFileSync('/tmp/dayoung-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
