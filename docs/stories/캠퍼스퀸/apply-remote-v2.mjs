// 원격 서버에서 실행 — 캠퍼스퀸 2차 풀-리라이트 반영
// /tmp/campus-v2-payload-story.json, /tmp/campus-v2-payload-lore-plan.json 을 읽어 localhost:8080 API 호출
// 순서: PUT story → POST creates(신규, 안전) → PUT updates → DELETE deletes(파괴적, 마지막)
// 이 2차 반영은 creates·deletes 0개라 사실상 PUT story(6필드) + PUT updates(7개)뿐.
// 결과: /tmp/campus-v2-apply-log.json
import fs from 'fs';
...
const story = JSON.parse(fs.readFileSync('/tmp/campus-v2-payload-story.json', 'utf8'));
const plan  = JSON.parse(fs.readFileSync('/tmp/campus-v2-payload-lore-plan.json', 'utf8'));
...
async function main() {
  ...
  // 4. DELETE deletes (파괴적 — 마지막) — 2차는 0개
...
function fin() {
  ...
  fs.writeFileSync('/tmp/campus-v2-apply-log.json', JSON.stringify(log, null, 2));
  console.log('\n=== 2차 적용 결과 ===');
